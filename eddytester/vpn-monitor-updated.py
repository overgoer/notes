...
SERVICES = [
    {"name": "amneziawg", "port": 51822, "type": "udp", "service": "wg-amnezia-interface"},
...
def check_ping():
    """Проверка ping до внешнего ресурса"""
    try:
...
def check_service(service_name):
    ...
    elif service_name == "wg-amnezia-interface":
        # Проверяем интерфейс AmneziaWG
        result = subprocess.run(
            ["ip", "link", "show", "wg-amnezia"],
            capture_output=True,
            text=True
        )
        return result.returncode == 0 and "UP" in result.stdout
...
def main():
    ...
    # Добавляем ping статус в корень JSON
    status["ping"] = {
        "status": ping_status,
        "timestamp": datetime.now().isoformat()
    }
