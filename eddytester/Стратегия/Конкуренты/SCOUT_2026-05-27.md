# SCOUT Report | 2026-05-27

## Competitor Activity Overview

| Channel | Posts | Top Categories |
|---------|-------|-----------------|
| qabigtech | 15 | AI/LLM, engagement, education |
| qachanell | 15 | API, engagement, guide |
| burning_tester | 15 | career, guide, API |
| rvtsakunov | 15 | engagement, other, practicum/portfolio |
| rvtsakunov_manual | 15 | career, engagement, API |
| protestinginfo | 15 | API, guide, AI/LLM |
| qa_chillout | 15 | API, engagement, automation |
| testerlib | 15 | engagement, API, career |
| serious_tester | 15 | API, engagement, career |
| qa_and_it | 15 | API, AI/LLM, bugs |

> Note: view/forward counts not available via Telethon for private channels. Analysis is category-based.

## Category Distribution per Channel

### qabigtech
Total posts: 15 | Images: 86.7%

| Category | Count | % |
|----------|-------|---|
| AI/LLM | 4 | 26.7% |
| engagement | 4 | 26.7% |
| education | 2 | 13.3% |
| API | 2 | 13.3% |
| guide | 1 | 6.7% |
| practicum/portfolio | 1 | 6.7% |
| bugs | 1 | 6.7% |

### qachanell
Total posts: 15 | Images: 73.3%

| Category | Count | % |
|----------|-------|---|
| API | 5 | 35.7% |
| engagement | 4 | 28.6% |
| guide | 4 | 28.6% |
| education | 1 | 7.1% |

### burning_tester
Total posts: 15 | Images: 93.3%

| Category | Count | % |
|----------|-------|---|
| career | 5 | 33.3% |
| guide | 2 | 13.3% |
| API | 2 | 13.3% |
| education | 2 | 13.3% |
| engagement | 1 | 6.7% |
| meme | 1 | 6.7% |
| bugs | 1 | 6.7% |
| other | 1 | 6.7% |

### rvtsakunov
Total posts: 15 | Images: 6.7%

| Category | Count | % |
|----------|-------|---|
| engagement | 8 | 57.1% |
| other | 3 | 21.4% |
| practicum/portfolio | 1 | 7.1% |
| career | 1 | 7.1% |
| education | 1 | 7.1% |

### rvtsakunov_manual
Total posts: 15 | Images: 46.7%

| Category | Count | % |
|----------|-------|---|
| career | 7 | 77.8% |
| engagement | 1 | 11.1% |
| API | 1 | 11.1% |

### protestinginfo
Total posts: 15 | Images: 93.3%

| Category | Count | % |
|----------|-------|---|
| API | 2 | 40.0% |
| guide | 1 | 20.0% |
| AI/LLM | 1 | 20.0% |
| engagement | 1 | 20.0% |

### qa_chillout
Total posts: 15 | Images: 100.0%

| Category | Count | % |
|----------|-------|---|
| API | 10 | 66.7% |
| engagement | 3 | 20.0% |
| automation | 1 | 6.7% |
| AI/LLM | 1 | 6.7% |

### testerlib
Total posts: 15 | Images: 86.7%

| Category | Count | % |
|----------|-------|---|
| engagement | 2 | 33.3% |
| API | 2 | 33.3% |
| career | 1 | 16.7% |
| bugs | 1 | 16.7% |

### serious_tester
Total posts: 15 | Images: 100.0%

| Category | Count | % |
|----------|-------|---|
| API | 8 | 53.3% |
| engagement | 3 | 20.0% |
| career | 2 | 13.3% |
| bugs | 2 | 13.3% |

### qa_and_it
Total posts: 15 | Images: 93.3%

| Category | Count | % |
|----------|-------|---|
| API | 5 | 55.6% |
| AI/LLM | 1 | 11.1% |
| bugs | 1 | 11.1% |
| other | 1 | 11.1% |
| engagement | 1 | 11.1% |

## Content Gaps (vs eddytester)

| Category | eddytester % | Competitor avg % | Gap |
|----------|-------------|-----------------|-----|
| API | 8.9% | 32.2% | +23.3% |

### Recommendations

- **API** (gap +23.3%) -- У конкурентов в этой теме: такое, разберём, вместо, статье, через

---
*Report generated at 2026-05-27 09:00:06*

## New Format Suggestions

Try these content formats on your audience (biweekly suggestion):

1. 🆬 **Short Video / Reels** — 30-60s bug-hunt breakdown ("How I found a bug in auth in 2 min")
2. 📊 **Comparison Tables** — Postman vs Insomnia, SQLite vs PostgreSQL for testing
3. 🧪 **Weekly Test Challenge** — "Find the bug in this API" with breakdown next post
4. 📋 **Cheat Sheets** — HTTP codes, SQL JOIN types, Git commands as card format
5. 🔍 **Real Case Study** — How a feature was tested end-to-end (with screenshots)
6. 🎯 **Audience Polls** — "Which DB is hardest to test?" — engages + gives content ideas
7. 🎚 **Monthly Resource Digest** — Collection of useful QA articles/tools

---
